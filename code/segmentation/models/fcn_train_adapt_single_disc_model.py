...
class FCNTrainAdaptSingleDiscModel(BaseModel):
    def name(self):
        return 'FCNTrainAdaptSingleDiscModel'

    def initialize(self, opt):
        BaseModel.initialize(self, opt)

        # specify the training losses you want to print out. The program will call base_model.get_current_losses
        self.loss_names = ['sem', 'edge', 'D_L', 'D_L_domain', 'D_domain']
        ...
        use_sigmoid = False
        self.netD_SR = networks.define_D(opt.output_nc + 2, opt.ndf, opt.which_model_netD, opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
    ...
    def optimize_parameters(self):  #update params
        ...
        self.pred_real = torch.cat( (self.pred_real_class, self.pred_real_edge), 1)
        self.D_out= self.netD_SR(self.pred_real)
        self.loss_D_L_domain = self.bce_loss(self.D_out, Variable(torch.FloatTensor(self.D_out.data.size()).fill_(0)).cuda())

        self.domain_loss_weight = 0.001
        self.loss_D_L_domain_weighted = self.domain_loss_weight * self.loss_D_L_domain
        ...
        pred_syn_concat = torch.cat( (pred_syn_class, pred_syn_edge), 1)
        pred_real_concat = torch.cat( (pred_real_class, pred_real_edge), 1)

        self.D_syn = self.netD_SR(pred_syn_concat)
        self.D_real = self.netD_SR(pred_real_concat)
